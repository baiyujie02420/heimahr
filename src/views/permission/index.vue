...
<script>
import { addPermission, getPermissionDetail, getPermissionList, updatePermission, delPermission } from '@/api/permisson'
...
export default {
  ...
  methods: {
    async getPermissionList() {
      ...
      this.list = transListToTreeData(await getPermissionList())
    },
    // 点击新增权限
    addPermission(id, type) {
      this.formData.pid = id // 把父的id给子的pid
      this.formData.type = type
      this.showDialog = true
    },
    // 点击编辑权限
    async editPermission(id) {
      this.formData = await getPermissionDetail(id) // 回显, 根据获取id获取详情
      this.showDialog = true
    },
    // 点击删除权限
    delPermission(id) {
    ...
    btnOK() {
      this.$refs.form.validate(async(isOk) => {
        if (isOk) {
          ...
          this.getPermissionList() // 重新获取权限列表
          this.showDialog = false
        }
      })
    }
